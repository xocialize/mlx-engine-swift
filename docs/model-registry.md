...
| imageUpscale | seedvr2-mlx-swift (MLXSeedVR2) | SeedVR2-3B | wrapper+core | video/PROD | 🧪 | 🟡 | ✅ | 0.17.0 | <!-- Export/diffusion tier of imageUpscale (Real-ESRGAN = fast tier; multi-package via PackageID). Folded born sweep-clean into the standalone core repo (EdgeTAM-style: SeedVR2MLX core + MLXSeedVR2 wrapper; tools 5.9→6.2, platforms→macOS26). License Apache-2.0/MIT (permissive, no ack). QuantConfigured fp16+int8 ONLY (int4 degrades 22.7 dB — not offered); BudgetAware drops fp16→int8 under pressure; unload()→clearCache; mmap/lazy (core eval()s up front, no full-dtype copy → floor≈on-disk). Split measured via seedvr2-package-smoke @256²→512²: int8 resident 4.72 GB / MLX-peak 7.07 GB (transient ~2.35 GB); fp16 resident est (not measured). Activation declared 3.5 GB conservatively — single-pass (no tiling V1) is resolution-driven; in-app phys re-baseline pending (smoke MLX-peak under-reads admission phys ~2.5–2.9×). Val 🟡 = CLI gate green (valid 512² out); MLXEngineVideo in-app run pending. Avail 🧪 = committed-local, wrapper not tagged/pushed. NB overlaps the older `videoUpscale | mlx-seedvr2-swift` WIP row — reconcile (fold video as a 2nd surface here, retire WIP). --> |
